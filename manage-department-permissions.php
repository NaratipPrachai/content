...
// คำสั่งแสดง error ทั้งหมด (สามารถเอาออกเมื่อใช้งานจริง)
...
// จัดการวิชาข้ามแผนก
if(isset($_GET['manage_cross_subjects']) && isset($_GET['user_id'])) {
    $manage_user_id = $_GET['user_id'];
    
    header("Location: manage-cross-subjects-detail.php?user_id=$manage_user_id");
    exit;
}
...
$permissions_sql = "SELECT ud.*, u.username, d.name as department_name,
                   ...
                   (SELECT COUNT(*) FROM user_subjects us JOIN subjects s ON us.subject_id = s.id 
                    WHERE us.user_id = ud.user_id AND s.department_id NOT IN 
                    (SELECT department_id FROM user_departments WHERE user_id = ud.user_id)) as cross_subjects
            ...
            <div class="p-6">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <?php if($permission['cross_subjects'] > 0): ?>
                                    <span class="px-2 py-1 bg-blue-100 text-blue-800 rounded-full text-xs ml-1">
                                        <i class="fas fa-exchange-alt mr-1"></i> <?php echo $permission['cross_subjects']; ?> วิชาข้ามแผนก
                                    </span>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex flex-col space-y-2">
                                        <a href="?manage_exclusion=1&user_id=<?php echo $permission['user_id']; ?>&department_id=<?php echo $permission['department_id']; ?>" 
                                           class="text-blue-500 hover:text-blue-700">
                                            <i class="fas fa-ban"></i> จัดการยกเว้นวิชา
                                        </a>
                                        
                                        <a href="?manage_cross_subjects=1&user_id=<?php echo $permission['user_id']; ?>" 
                                           class="text-green-500 hover:text-green-700">
                                            <i class="fas fa-exchange-alt"></i> จัดการวิชาข้ามแผนก
                                        </a>
                                        ...
                                        <a href="?delete_user_id=<?php echo $permission['user_id']; ?>&delete_department_id=<?php echo $permission['department_id']; ?>" 
                                           ...
                                           onclick="return confirm('ยืนยันการลบสิทธิ์นี้? การลบสิทธิ์สาขาวิชานี้จะทำให้ผู้ใช้หมดสิทธิ์เข้าถึงรายวิชาทั้งหมดในสาขานี้ด้วย')">
                                            <i class="fas fa-trash-alt"></i> ลบสิทธิ์
                                        </a>
                                    </div>
                                </td>
                            </tr>
                ...
                <div class="mt-6">
                    <div class="bg-blue-50 border-l-4 border-blue-500 text-blue-700 p-4">
                        <h4 class="font-bold">คำแนะนำการใช้งาน</h4>
                        <ul class="list-disc ml-6 mt-2">
                            <li>การจัดการยกเว้นวิชา - ใช้สำหรับยกเว้นบางวิชาไม่ให้ผู้ใช้เข้าถึง แม้ว่าจะมีสิทธิ์ในสาขาวิชานั้น</li>
                            <li>การจัดการวิชาข้ามแผนก - ใช้สำหรับให้สิทธิ์ผู้ใช้เข้าถึงบางวิชาในแผนกที่ผู้ใช้ไม่มีสิทธิ์ทั้งแผนก เช่น นักศึกษาในแผนกคอมพิวเตอร์ที่ต้องเรียนวิชาบัญชีเบื้องต้นเพื่อปรับพื้นฐาน</li>
                            <li>ระบบจะจัดการสิทธิ์การเข้าถึงตามลำดับ: ดูสิทธิ์แผนกก่อน > ตรวจสอบการยกเว้น > ตรวจสอบสิทธิ์วิชาข้ามแผนก</li>
                        </ul>
                    </div>
                </div>
            </div>
            ...
            <form method="POST">
                ...
                <div class="mb-4">
                    ...
                    <p class="text-xs text-gray-500 mt-1">
                        ...
                        * คุณสามารถยกเว้นบางวิชาไม่ให้เข้าถึงได้ในภายหลัง<br>
                        * สำหรับนักศึกษาที่ต้องเรียนบางวิชาจากแผนกอื่น ให้ใช้ "จัดการวิชาข้ามแผนก"
                    </p>
                </div>
